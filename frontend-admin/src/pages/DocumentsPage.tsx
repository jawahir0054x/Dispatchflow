...
export function DocumentsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedLoad && (
        <div className="rounded-xl border border-surface-800 bg-surface-900 px-4 py-3 text-sm text-slate-400">
          <span className="font-medium text-white">{selectedLoad.loadNumber}</span>
          {' · '}
      ...
      {selectedLoadId !== '' && (
        <>
          <form
            ...
            className="flex flex-wrap items-end gap-4 rounded-2xl border border-surface-800 bg-surface-900 p-4 shadow-sm"
          >
            ...
            <FormField label="File (PDF or image, max 10MB)">
              <input
                ...
                className="block w-full min-w-64 text-sm text-slate-400 file:mr-3 file:rounded-lg file:border-0 file:bg-brand-950 file:px-3 file:py-2 file:text-sm file:font-semibold file:text-brand-300 hover:file:bg-brand-900"
                required
              />
            </FormField>
            <button
              ...
              <table className="min-w-full text-left text-sm">
                ...
                <tbody>
                  {loadingDocuments ? (
                    ...
                    documents.map((doc) => (
                      <tr key={doc.id} className="border-b border-surface-800 last:border-0">
                        <td className="px-4 py-3 font-medium text-white">
                          {documentTypeLabel(doc.documentType)}
                        </td>
                        <td className="px-4 py-3 text-slate-300">{doc.originalFilename}</td>
                        <td className="px-4 py-3 text-slate-400">{formatFileSize(doc.fileSize)}</td>
      ...
      >
        {previewLoading && <p className="py-8 text-center text-sm text-slate-400">Loading preview...</p>}
          ...
          <div className="max-h-[70vh] overflow-auto">
            {previewDoc.contentType === 'application/pdf' ? (
              <iframe
                ...
                className="h-[65vh] w-full rounded-lg border border-surface-700"
              />
            ) : (
              <img
                src={previewUrl}
